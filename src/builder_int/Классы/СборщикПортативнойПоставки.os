...
Процедура ПриСозданииОбъекта(Каталог)
	...
	ПрефиксПеременныхИПроцедур = "UT_GENERATION_";
	...
	МодулиБезДобавленияВИнтерфейс.Добавить("UT_Support");
	МодулиБезДобавленияВИнтерфейс.Добавить("UT_JSONDataConversion");
	МодулиБезДобавленияВИнтерфейс.Добавить("UT_SettingsStoragesManager");
	...
	ТекстыСтроковыхФункцияДляСовместимости.Вставить("МодульФормы", ДобавитьДирективуМетодовМодуля(ТекстФункцийСовместимости,"&AtClientAtServerNoContext"));
...
Процедура ВыполнитьКонвертацию(КаталогПортативнойСборки) Экспорт
	...
	ПрочитатьОписаниеКонфигурации(ОбъединитьПути(КаталогИсходныхФайловРасширения, "ToolsInternational"));
...
Процедура СкопироватьМодульМенеджераВоВнешнююОбработку(ОписаниеМодуля)
	...
	ИмяМодуляМенеджера=ОписаниеМодуля.Имя + "_ManagerModule";
...
Процедура СкопироватьИсходныеФайлыДляКонвертации()
	...
	Для Каждого ТекКартинка Из ОписаниеРасширения.ОбщиеКартинки Цикл
		КаталогПоискаФайла=ОбъединитьПути(ОбъединитьПути(КаталогИсходныхФайловРасширения,"Tools","CommonPictures",ТекКартинка.Имя),"Ext","Picture");
	...
	КаталогГлавнойОбработки = ОбъединитьПути(СтруктураПортативныхИнструментов.КаталогИсходников, "Portable");
	
	КаталогИсходныхФайловГлавнойОбработки = ОбъединитьПути(КаталогИсходныхФайловРасширения, "Portable", "ExternalDataProcessors");
...
Процедура ПрочитатьОписаниеКонфигурации(КаталогИсходниковИнструментов)
	...
	Для Каждого ТекМодуль Из ОбщиеМодулиРасширения Цикл
		Если Не СтрНачинаетсяС(НРег(ТекМодуль.Имя), "ut_") Тогда
			Продолжить;
		КонецЕсли;
...
Процедура СконвертироватьМодулиПортативнойПоставки()
	...
	ИмяМодуляПрограммногоИнтерфейса = "UT_";
	ИмяФайлаФормы=ОбъединитьПути(
...
Процедура СконвертироватьМодуль(ИмяФайлаМодуля, ЭтоМодульФормы, ЭтоГлавнаяОбработка=Ложь, ЭтоОбщийМодуль=Ложь, ЭтоКлиентскийОбщийМодуль = Ложь)
	...
	Если ЭтоМодульФормы И Не ЭтоГлавнаяОбработка Тогда
		...
		СтруктураПереименованийПеременныхМодулей.Вставить("UT_ApplicationParameters", ПрефиксПеременныхИПроцедур+"UT_ApplicationParameters()");
...
Процедура ДобавитьВГлавнуюОбработкуМодульПрограммногоИнтерфейса(УзелОбработки)
	...
	Для Каждого ТекМодуль Из ОписаниеРасширения.ОбщиеМодули Цикл
		Если НРег(ТекМодуль.Имя)="UT_" Тогда
...
Процедура СобратьБинарныеФайлыПроекта(КаталогПортативнойСборки)
	...
	КаталогОбработокСборки = ОбъединитьПути(КаталогПортативнойСборки, "ToolsInternational");
	ФС.ОбеспечитьКаталог(КаталогОбработокСборки);
	КаталогОбщихМодулейСборки=ОбъединитьПути(КаталогПортативнойСборки, "CommonModules");
...
ИмяГлавнойОбработки = "UT_PortableUniversalTools";
