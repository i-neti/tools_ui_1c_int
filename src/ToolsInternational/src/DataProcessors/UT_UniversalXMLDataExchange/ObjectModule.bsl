...
Процедура ИнициализироватьВедениеПротоколаОбмена() Экспорт
	...
	Если ПустаяСтрока(ExchangeProtocolFileName) Тогда
	...
	мФайлПротоколаДанных = Новый ЗаписьТекста(ExchangeProtocolFileName, КодировкаФайлаПротоколаОбмена(), ,
...
Функция КодировкаФайлаПротоколаОбмена()
	...
	ПредставлениеКодировки = СокрЛП(ExchangeProtocolFileEncoding);

	Результат = КодировкаТекста.ANSI;
	Если Не ПустаяСтрока(ExchangeProtocolFileEncoding) Тогда
...
//	КонецЕсли;
	...
	Если Правило.Отбор <> Неопределено Тогда
		...
		UT_CommonClientServer.SetDCSParemeterValue(КомпоновщикНастроек, "ДатаНачала", ДатаНачала);
...
Процедура ОткрытьФайлЗагрузки(ТолькоПрочитатьШапку = Ложь, ДанныеФайлаОбмена = "") Экспорт
	...
	Если ИмяУзла = "ПравилаОбмена" Тогда
		Если SafeImport И ЗначениеЗаполнено(ИмяФайлаПравилОбмена) Тогда
...
Процедура СжатьРезультирующийФайлОбмена()
	...
	Попытка
		...
		Архиватор = Новый ЗаписьZipФайла(ExchangeFileName, ПарольДляСжатияФайлаОбмена, НСтр("ru = 'Файл обмена данными'"));
...
Функция ПолучитьИмяПротоколаДляВторойИнформационнойБазыComСоединения() Экспорт
	...
	ИначеЕсли Не ПустаяСтрока(ExchangeProtocolFileName) Тогда

		РасширениеФайлаПротокола = ПолучитьРасширениеИмениФайла(ExchangeProtocolFileName);

		Если Не ПустаяСтрока(РасширениеФайлаПротокола) Тогда

			ИмяФайлаПротоколаВыгрузки = СтрЗаменить(ExchangeProtocolFileName, "." + РасширениеФайлаПротокола, "");
