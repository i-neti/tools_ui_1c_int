...
Функция ПолучитьСтрокуОграниченияПоДатеДляЗапроса(Свойства, ИмяТипа, ИмяГруппыТаблицы = "",
	...
	Если StartDate <> ЗначениеПустаяДата Тогда

		ИтоговоеОграничениеПоДате = "
									|	ГДЕ
									|		" + ИмяПоляОграничения + " >= &StartDate";

	КонецЕсли;

	Если EndDate <> ЗначениеПустаяДата Тогда

		Если ПустаяСтрока(ИтоговоеОграничениеПоДате) Тогда

			ИтоговоеОграничениеПоДате = "
										|	ГДЕ
										|		" + ИмяПоляОграничения + " <= &EndDate";
...
Функция ПолучитьРезультатЗапросаДляВыгрузкиОчисткиДанных(Свойства, ИмяТипа, ВыборкаДляОчисткиДанных = Ложь,
	...
	Если ИмяТипа = "Справочник" Или ИмяТипа = "ПланВидовХарактеристик" Или ИмяТипа = "ПланСчетов" Или ИмяТипа = "ПланВидовРасчета"
		...
		Иначе
			...
			Если ExportAllowedObjectsOnly И Не ВыбиратьВсеПоля Тогда
	...
	ИначеЕсли ИмяТипа = "Документ" Тогда
		...
		Если ExportAllowedObjectsOnly Тогда
		...
		Запрос.УстановитьПараметр("StartDate", StartDate);
		Запрос.УстановитьПараметр("EndDate", EndDate);
...
Процедура ИнициализацияВнешнейОбработкиОбработчиковСобытий(РаботаВозможна, ОбъектВладелец) Экспорт
	...
	Если HandlersDebugModeFlag И ПустаяСтрока(EventHandlerExternalDataProcessorFileName) Тогда

		ЗаписатьВПротоколВыполнения(77);
		РаботаВозможна = Ложь;

	ИначеЕсли HandlersDebugModeFlag Тогда

		Попытка

			Если ЭтоВнешняяОбработка() Тогда

				ВнешняяОбработкаОбработчиковСобытий = ВнешниеОбработки.Создать(EventHandlerExternalDataProcessorFileName, Ложь);

			Иначе

				ВнешняяОбработкаОбработчиковСобытий = Обработки[EventHandlerExternalDataProcessorFileName].Создать();
