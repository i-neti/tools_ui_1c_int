...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.ExchangeFileName = Параметры.ИмяФайлаОбмена;
	Объект.ExchangeRulesFileName = Параметры.ExchangeRulesFileName;
	Объект.EventHandlerExternalDataProcessorFileName = Параметры.EventHandlerExternalDataProcessorFileName;
	Объект.AlgorithmDebugMode = Параметры.AlgorithmDebugMode;
	Объект.ReadEventHandlersFromExchangeRulesFile = Параметры.ReadEventHandlersFromExchangeRulesFile;
...
Процедура ИмяФайлаВнешнейОбработкиОбработчиковСобытийНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Оповещение = Новый ОписаниеОповещения("ИмяФайлаВнешнейОбработкиОбработчиковСобытийОбработкаВыбора", ЭтотОбъект,
...
Процедура ИмяФайлаВнешнейОбработкиОбработчиковСобытийОбработкаВыбора(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Объект.EventHandlerExternalDataProcessorFileName = ВыбранныеФайлы[0];
...
Процедура Готово(Команда)
	...
	Если ПустаяСтрока(Объект.EventHandlerExternalDataProcessorFileName) Тогда
	...
	ФайлВнешнейОбработкиОбработчиковСобытий = Новый Файл(Объект.EventHandlerExternalDataProcessorFileName);

	Оповещение = Новый ОписаниеОповещения("ПроверкаСуществованияФайлаВнешнейОбработкиОбработчиковСобытийЗавершение",
		ЭтотОбъект);
...
Процедура ПроверкаСуществованияФайлаВнешнейОбработкиОбработчиковСобытийЗавершение(Существует, ДополнительныеПараметры) Экспорт
	...
	ПараметрыЗакрытия.Вставить("EventHandlerExternalDataProcessorFileName",
		Объект.EventHandlerExternalDataProcessorFileName);
	ПараметрыЗакрытия.Вставить("AlgorithmDebugMode", Объект.AlgorithmDebugMode);
	ПараметрыЗакрытия.Вставить("ExchangeRulesFileName", Объект.ExchangeRuleFileName);
	ПараметрыЗакрытия.Вставить("ИмяФайлаОбмена", Объект.ExchangeFileName);
...
Процедура УстановитьВидимость()
	...
	УстановитьВыделениеИмениФайлаВнешнейОбработки(ПустаяСтрока(Объект.EventHandlerExternalDataProcessorFileName));

	Элементы.ОткрытьФайл.Доступность = Не ПустаяСтрока(Объект.EventHandlersTempFileName);
...
Процедура ВыгрузитьКодОбработчиков(Команда)
	...
	Если Не ПустаяСтрока(Объект.EventHandlersTempFileName) Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Модуль отладки с кодом обработчиков уже выгружен.'"),
			СписокКнопок, , КодВозвратаДиалога.Нет);
...
Функция ОбработчикиСобытий()
	...
	ФайлОбработчиков = Новый Файл(Объект.EventHandlersTempFileName);
	Если ФайлОбработчиков.Существует() И ФайлОбработчиков.Размер() <> 0 Тогда
		ОбработчикиСобытий = Новый ТекстовыйДокумент;
		ОбработчикиСобытий.Прочитать(Объект.EventHandlersTempFileName);
	КонецЕсли;
...
Функция ПротоколОбмена()
	...
	ФайлОшибок = Новый Файл(Объект.ExchangeLogTempFileName);
	Если ФайлОшибок.Существует() И ФайлОшибок.Размер() <> 0 Тогда
		ПротоколОбмена = Новый ТекстовыйДокумент;
		ПротоколОбмена.Прочитать(Объект.EventHandlersTempFileName);
	КонецЕсли;
...
Процедура ПриИзмененииРежимаОтладкиИзменений()
	...
	Подсказка.ТекущаяСтраница = Подсказка.ПодчиненныеЭлементы["Группа_" + Объект.AlgorithmDebugMode];
