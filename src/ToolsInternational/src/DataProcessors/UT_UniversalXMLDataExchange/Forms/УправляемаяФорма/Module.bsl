...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.ExchangeProtocolFileEncoding = "КодировкаТекста.UTF8";
...
Процедура ИмяФайлаПротоколаОбменаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ВыборФайла(Элемент, Объект, "ExchangeProtocolFileName", Ложь, "txt", Ложь);
...
Процедура АутентификацияWindowsИнформационнойБазыДляПодключенияПриИзменении(Элемент)
	...
	Элементы.ПользовательИнформационнойБазыДляПодключения.Доступность = Не Объект.InfobaseConnectionWindowsAuthentification;
	Элементы.ПарольИнформационнойБазыДляПодключения.Доступность = Не Объект.InfobaseConnectionWindowsAuthentification;
...
Процедура АрхивироватьФайлПриИзмененииЗначения()
	...
	Элементы.ПарольДляСжатияФайлаОбмена.Доступность = Объект.ArchiveFile;
...
#Если Не ВебКлиент Тогда
	...
	Если Не ПустаяСтрока(Объект.ExchangeProtocolFileName) Тогда
		ОткрытьВПриложении(Объект.ExchangeProtocolFileName);
	КонецЕсли;
