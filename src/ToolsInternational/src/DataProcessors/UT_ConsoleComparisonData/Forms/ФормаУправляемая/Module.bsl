...
Функция ПолучитьТабличныйДокументСДаннымиИзИсточникаНаСервере(BaseID, МаксимальноеЧислоСтрок = 0, ТолькоДубликаты = Ложь, Подключение = Неопределено)
	...
	ТЗ_Сгруппированная.Свернуть(КолонкиСКлючомСтрокой, ColumnNameNumberOfRowsDataSource);
...
Процедура ОбновитьВидимостьДоступностьЭлементовФормыПоИдентификаторуБазы(BaseID)
	...
	ИначеЕсли Объект["BaseType" + BaseID] = 3 Тогда
		...
		ФайлФорматаXML = Объект["ConnectionToExternalBase" + BaseID + "ФорматФайла" ] = "XML";
		ФайлФорматаXLS = Объект["ConnectionToExternalBase" + BaseID + "ФорматФайла" ] = "XLS";
		ФайлФорматаDOC = Объект["ConnectionToExternalBase" + BaseID + "ФорматФайла" ] = "DOC";
		...
		Элементы["ConnectionToExternalBase" + BaseID + "НомерТаблицыВФайле"].Видимость		= ФайлФорматаXLS Или ФайлФорматаDOC;
		Элементы["ConnectionToExternalBase" + BaseID + "НомерТаблицыВФайле"].Title		= ?(ФайлФорматаXLS, "Номер книги", "Номер таблицы");
	...
	ИначеЕсли Объект["BaseType" + BaseID] = 4 Тогда
		...
		Элементы["ConnectionToExternalBase" + BaseID + "НомерТаблицыВФайле"].Видимость		= Ложь;
	...
	ИначеЕсли Объект["BaseType" + BaseID] = 6 Тогда
		...
		Элементы["ConnectionToExternalBase" + BaseID + "НомерТаблицыВФайле"].Видимость		= Ложь;
		Элементы["ConnectionToExternalBase" + BaseID + "НомерТаблицыВФайле"].Title		= Ложь;
