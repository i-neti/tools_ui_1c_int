...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.UseFastInfoSetFormat = (Файл.Расширение = ".fi");
...
Процедура ИспользоватьФорматFastInfoSetПриИзменении(Элемент)
	...
	Если Объект.UseFastInfoSetFormat Тогда
...
Процедура ДеревоМетаданныхВыгружатьПриИзменении(Элемент)
	...
	ТекущиеДанныеИмя = Элементы.MetadataTree.ТекущиеДанные.ПолноеИмяМетаданных;
...
Процедура ОчиститьДополнительныеОбъектыВыгрузки(Команда)
	...
	Объект.AdditionalObjectsToExport.Очистить();
...
Процедура ВыгрузитьДанные(Команда)
	...
	Объект.StartDate = ПериодВыгрузки.StartDate;
	Объект.EndDate = ПериодВыгрузки.EndDate;
	...
	Если РежимРаботыНаКлиенте И Не ПустаяСтрока(АдресФайлаВоВременномХранилище) Тогда
		...
		ИмяФайла = ?(Объект.UseFastInfoSetFormat, Нстр("ru = 'Файл выгрузки.fi'"), Нстр(
...
Процедура ПересчитатьВыгружаемыеПоСсылке(Команда)
	...
	СохранитьОтображениеДерева(Объект.MetadataTree.ПолучитьЭлементы());
	ПересчитатьВыгружаемыеПоСсылкеНаСервере();
	ВосстановитьОтображениеДерева(Объект.MetadataTree.ПолучитьЭлементы());
...
Процедура ВыгрузитьДанныеНаСервере(АдресФайлаВоВременномХранилище)
	...
	Если РежимРаботыНаКлиенте Тогда
		...
		Расширение = ?(Объект.UseFastInfoSetFormat, ".fi", ".xml");
...
Процедура ОбработкаВыбораНаСервере(ВыбранныеЗначения)
	...
	Для Каждого Значение Из ВыбранныеСсылки Цикл
		...
		НоваяСтрока = Объект.AdditionalObjectsToExport.Добавить();
...
Процедура ИнициализироватьСКД()
	...
	Объект.Composer.Инициализировать(ИсточникНастроек);

	Объект.Composer.ЗагрузитьНастройки(СхемаКомпоновкиДанных.НастройкиПоУмолчанию);

	Если ТипОбъекта = "Документы" Тогда

		НовоеПолеСортировки = Объект.Composer.Настройки.Порядок.Элементы.Добавить(Тип(
...
Процедура ПодготовитьСписокВыбранныхОбъектов(ТекущиеДанныеИмя, ИмяОбъектаМетаданных)
	...
	КэшОтбора = Объект.Composer.Настройки.Отбор.Элементы;
	...
	ВосстановитьОтборИзКэша(Объект.Composer.Настройки.Отбор.Элементы, КэшОтбора,
		Объект.Composer.Настройки.Отбор.ДоступныеПоляОтбора);
...
Процедура СкопироватьОтборнаСервере(ТекущиеДанныеИмя)
	...
	Дерево1 = Объект.MetadataTree.ПолучитьЭлементы();
...
Процедура ОбновитьОтборПоАктивизацииНаСервере(ТекущиеДанныеИмя, ИмяОбъектаМетаданных)
	...
	Отбор = Объект.Composer.Настройки.Отбор;
...
Процедура ОтборПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если Не ОтменаРедактирования Тогда
		...
		Если Выделить И Не Элементы.MetadataTree.ТекущиеДанные.Выделить Тогда
			ЭтаФорма.Элементы.MetadataTree.ТекущиеДанные.ИндексКартинки = ЭтаФорма.Элементы.MetadataTree.ТекущиеДанные.ИндексКартинки
				+ 1;
	...
	Если КэшОтбора.Количество() > 0 Тогда
		...
		Для Каждого ЭлементОтбора Из КэшОтбора Цикл
			...
			Иначе
				...
				Если Объект.Composer.Настройки.Отбор.ДоступныеПоляОтбора.НайтиПоле(ЭлементОтбора.ЛевоеЗначение)
					<> Неопределено Тогда
...
Функция ОбновитьТаблицуОтборанаСервере(ПолноеИмяМетаданных, ИмяОбъектаМетаданных)
	...
	ОтборСсылка = Объект.Composer.Настройки.Отбор;
...
// Вставить содержимое обработчика.
	...
	Объект.Composer.Настройки.Отбор.Элементы.Очистить();
...
//ЭтаФорма.Элементы.MetadataTree.ТекущиеДанные.ИндексКартинки=4;
