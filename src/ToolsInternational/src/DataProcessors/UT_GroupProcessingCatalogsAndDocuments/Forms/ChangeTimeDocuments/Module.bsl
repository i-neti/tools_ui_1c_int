...
Procedure ProcessObject(Reference, SequenceNumberObject, ParametersWriteObjects)
	...
	If KindChangesTime = "AtStartOfTheDay" Then
		ProcessedObject.Date = BegOfDay(Reference.Date);
	ElsIf KindChangesTime = "AtTheEndOfTheDay" Then
		ProcessedObject.Date = EndOfDay(Reference.Date);
	Else
		ProcessedObject.Date = BegOfDay(Reference.Date) + (InputTime - '00010101');
	EndIf;
...
&AtClient
Procedure KindChangesTimeOnChange(Item)
	SetAccessibility();
EndProcedure

&AtClient
Procedure SetAccessibility()
	Items.InputTime.Visible = KindChangesTime = "EnterManually";
EndProcedure
...
mSetting.Insert("KindChangesTime", "AtTheEndOfTheDay");
mSetting.Insert("InputTime");
