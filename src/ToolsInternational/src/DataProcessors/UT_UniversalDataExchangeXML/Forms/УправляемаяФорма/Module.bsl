...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.IsInteractiveMode = Истина;
	Объект.SafeMode = Истина;
	Объект.ExchangeLogFileEncoding = "КодировкаТекста.UTF8";
...
Процедура ПриОткрытии(Отказ)
	...
	ПрямаяВыгрузка = ?(Объект.DirectReadingFromDestinationIB, 1, 0);

	СохраненныйРежимЗагрузки = (Объект.ExchangeMode = "Загрузка");
...
#Если ВебКлиент Тогда
	...
	Объект.HandlersDebugModeFlag = Ложь;
#КонецЕсли

	УстановитьДоступностьКомандОтладки();

	Если СохраненныйРежимЗагрузки И Объект.AutomaticDataImportSettings <> 0 Тогда

		Если Объект.AutomaticDataImportSettings = 1 Тогда
...
Процедура ПриОткрытииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		ПредставлениеПериодаВыгрузки = ПредставлениеПериода(Объект.StartDate, Объект.EndDate);
...
Процедура ГлавнаяПанельФормыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ТекущаяСтраница.Имя = "Выгрузка" Тогда
		...
		Объект.ExchangeMode = "Выгрузка";

	ИначеЕсли ТекущаяСтраница.Имя = "Загрузка" Тогда

		Объект.ExchangeMode = "Загрузка";
...
Процедура ФлагРежимОтладкиПриИзменении(Элемент)
	...
	Если Объект.DebugModeFlag Тогда

		Объект.UseTransactions = Ложь;
...
Процедура КоличествоОбработанныхОбъектовДляОбновленияСтатусаПриИзменении(Элемент)
	...
	Если Объект.ProcessedObjectsCountToUpdateStatus = 0 Тогда
		Объект.ProcessedObjectsCountToUpdateStatus = 100;
	КонецЕсли;
...
Процедура ИмяФайлаОбменаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ВыборФайла(Элемент, ЭтотОбъект, "ИмяФайлаОбмена", Ложь, , Объект.ArchiveFile);
...
Процедура ИмяФайлаПротоколаОбменаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ВыборФайла(Элемент, Объект, "ExchangeLogFileName", Ложь, "txt", Ложь);
...
Процедура ИмяФайлаПротоколаОбменаЗагрузкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ВыборФайла(Элемент, Объект, "ImportExchangeLogFileName", Ложь, "txt", Ложь);
...
Процедура ИмяФайлаДанныхНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ВыборФайла(Элемент, ЭтотОбъект, "ИмяФайлаДанных", Ложь, , Объект.ArchiveFile);
...
Процедура КаталогИнформационнойБазыДляПодключенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогВыбораФайла.Каталог = Объект.InfobaseConnectionDirectory;
...
Процедура КаталогИнформационнойБазыДляПодключенияОбработкаВыбора(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Объект.InfobaseConnectionDirectory = ВыбранныеФайлы[0];
...
Процедура АутентификацияWindowsИнформационнойБазыДляПодключенияПриИзменении(Элемент)
	...
	Элементы.InfobaseConnectionUsername.Доступность = Не Объект.InfobaseConnectionWindowsAuthentification;
	Элементы.InfobaseConnectionPassword.Доступность = Не Объект.InfobaseConnectionWindowsAuthentification;
...
Процедура ИмяФайлаДанныхПриИзменении(Элемент)
	...
	Объект.ExchangeFileName = ИмяФайлаДанных;

	Файл = Новый Файл(Объект.ExchangeFileName);
	Объект.ArchiveFile = (ВРЕГ(Файл.Расширение) = ВРЕГ(".zip"));
...
Процедура ВерсияПлатформыИнформационнойБазыДляПодключенияПриИзменении(Элемент)
	...
	Если ПустаяСтрока(Объект.PlatformVersionForInfobaseConnection) Тогда

		Объект.PlatformVersionForInfobaseConnection = "V8";
...
Процедура ВыгрузкаОтметитьВсе(Команда)
	...
	Для Каждого Строка Из Объект.ExportRulesTable.ПолучитьЭлементы() Цикл
		Строка.Включить = 1;
...
Процедура НастройкаОтладкиВыгрузки(Команда)
	...
	Объект.ExchangeRulesFileName = ИмяФайлаНаСервереИлиКлиенте(ИмяФайлаПравил, АдресФайлаПравилВХранилище);
...
Процедура НастройкаОтладкиЗагрузкиЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Объект.ExchangeFileName = ИмяФайлаНаСервереИлиКлиенте(ИмяФайлаОбмена, Адрес, ВыбранноеИмяФайла);
...
Процедура УИ_ТестПодключенияКВебСервису(Команда)
	Путь=Объект.UT_DestinationPublicationAddress + "/hs/tools-ui-1c/exchange";

	ДопПараметры=Новый Структура;
	ДопПараметры.Вставить("Таймаут", 10);

	Аутентификация=Новый Структура;
	Аутентификация.Вставить("Пользователь", Объект.InfobaseConnectionUsername);
	Аутентификация.Вставить("Пароль", Объект.InfobaseConnectionPassword);
...
Процедура ОчиститьДанныеОФайлеДляЗагрузкиДанных()
	...
	Объект.ExchangeRulesVersion = "";
	Объект.DataExportDate = "";
...
Процедура ОбработатьДоступностьЭлементовУправленияТранзакциями()
	...
	Элементы.UseTransactions.Доступность = Не Объект.DebugModeFlag;

	Элементы.ObjectsPerTransaction.Доступность = Объект.UseTransactions;
...
Процедура АрхивироватьФайлПриИзмененииЗначения()
	...
	Если Объект.ArchiveFile Тогда
		ИмяФайлаДанных = СтрЗаменить(ИмяФайлаДанных, ".xml", ".zip");
	Иначе
		ИмяФайлаДанных = СтрЗаменить(ИмяФайлаДанных, ".zip", ".xml");
	КонецЕсли;

	Элементы.ExchangeFileCompressionPassword.Доступность = Объект.ArchiveFile;
...
Процедура ВыполнитьЗагрузкуПравилОбмена(АдресФайлаПравилВХранилище = "", ИмяФайлаДляРасширения = "")
	...
	Иначе
		...
		УстановитьПризнакЗагрузкиПравил(Истина);
		РазвернутьСтрокиДерева(Объект.ExportRulesTable, Элементы.ТаблицаПравилВыгрузки, "Включить");
...
Процедура ПриИзмененииПериода()
	...
	Объект.StartDate = ПериодВыгрузки.ДатаНачала;
	Объект.EndDate = ПериодВыгрузки.ДатаОкончания;
...
Процедура ЗагрузитьПравилаОбменаИПараметрыНаСервере(АдресФайлаПравилВХранилище, ИмяФайлаДляРасширения)
	...
	Иначе
		...
		Объект.ExchangeRulesFileName = ИмяФайлаПравилОбмена;

	КонецЕсли;

	ОбъектДляСервера = РеквизитФормыВЗначение("Объект");
	ОбъектДляСервера.ExportRulesTable = РеквизитФормыВЗначение("Объект.ExportRulesTable");
	ОбъектДляСервера.ParametersSettingsTable = РеквизитФормыВЗначение("Объект.ParametersSettingsTable");
	...
	Объект.ErrorFlag = ОбъектДляСервера.ErrorFlag;

	Если ЭтоКлиент Тогда

		УдалитьФайлы(Объект.ExchangeRuleFileName);

	КонецЕсли;

	ЗначениеВРеквизитФормы(ОбъектДляСервера.ExportRulesTable, "Объект.ExportRulesTable");
	ЗначениеВРеквизитФормы(ОбъектДляСервера.ParametersSettingsTable, "Объект.ParametersSettingsTable");
...
Процедура ВыборФайла(Элемент, ОбъектХранения, ИмяСвойства, ПроверятьСуществование, Знач РасширениеПоУмолчанию = "xml",
	...
	ИначеЕсли Объект.ExchangeMode = "Выгрузка" Тогда
...
Процедура ОткрытьФайлЗагрузкиНаСервере(АдресФайла)
	...
	Если ЭтоКлиент Тогда
		...
		Объект.ExchangeFileName = АдресНаСервере;
	...
	Иначе
		...
		Объект.ExchangeFileName = ИмяФайлаОбмена;
	...
	Объект.StartDate = ОбъектДляСервера.StartDate;
	Объект.EndDate = ОбъектДляСервера.EndDate;
	Объект.DataExportDate = ОбъектДляСервера.DataExportDate;
	Объект.ExchangeRulesVersion = ОбъектДляСервера.ExchangeRulesVersion;
	Объект.Comment = ОбъектДляСервера.Comment;
...
Процедура УстановитьУзелОбменаУСтрокДереваНаСервере(УзелОбмена)
	...
	УстановитьУзелОбменаУСтрокДерева(Объект.ExportRulesTable.ПолучитьЭлементы(), УзелОбмена);
...
Процедура СохранитьПараметрыНаСервере()
	...
	ТаблицаПараметров = РеквизитФормыВЗначение("Объект.ParametersSettingsTable");
...
Процедура ВосстановитьПараметрыНаСервере()
	...
	ЗначениеВРеквизитФормы(ТаблицаПараметров, "Объект.ParametersSettingsTable");
...
Процедура ВыполнитьЗагрузкуНаСервере(АдресФайла, ИмяФайлаДляРасширения)
	...
	Иначе
		...
		Объект.ExchangeFileName = ИмяЗагружаемогоФайла;

	КонецЕсли;

	Если Объект.SafeImport Тогда
		Если ЭтоАдресВременногоХранилища(АдресФайлаПравилЗагрузкиВХранилище) Тогда
			...
			АдресНаСервере = ПолучитьИмяВременногоФайла("xml");
			// Удаление временного файла происходит не через УдалитьФайлы(АдресНаСервере), а через
			// УдалитьФайлы(Объект.ExchangeRulesFileName) далее.
			ДвоичныеДанные.Записать(АдресНаСервере);
			Объект.ExchangeRulesFileName = АдресНаСервере;
		Иначе
...
Функция ИмяФайлаНаСервереИлиКлиенте(ИмяРеквизита, Знач АдресФайла, Знач ИмяФайлаДляРасширения = ".xml",
	...
	Если ЭтоКлиент Тогда
		...
		Если СоздатьНовый Тогда
			...
			Расширение = ?(Объект.ArchiveFile, ".zip", ".xml");
		...
		Иначе
			...
			// а через УдалитьФайлы(Объект.ExchangeRulesFileName) и УдалитьФайлы(Объект.ИмяФайлаОбмена) далее.
			ДвоичныеДанные.Записать(АдресНаСервере);
...
Процедура ВыполнитьВыгрузкуИзФормы()
	...
	Если Не Объект.DirectReadFromDestinationIB И Не ЭтоКлиент Тогда
	...
	Если ЭтоКлиент И Не ПрямаяВыгрузка И Не Объект.ErrorFlag Тогда

		ИмяСохраняемогоФайла = ?(Объект.ArchiveFile, НСтр("ru = 'Файл выгрузки.zip'"), НСтр(
...
Функция ВыполнитьВыгрузкуНаСервере()
	...
	Если Не ПрямаяВыгрузка Тогда
		...
		Иначе
			...
			Объект.ExchangeFileName = ИмяВременногоФайлаДанных;
	...
	ТаблицаПравилВыгрузки = РеквизитФормыВЗначение("Объект.ExportRulesTable");
	ТаблицаНастройкиПараметров = РеквизитФормыВЗначение("Объект.ParametersSettingsTable");
	...
	Если ЭтоКлиент И Не ПрямаяВыгрузка Тогда
		...
		АдресФайлаДанных = ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(Объект.ExchangeFileName),
			УникальныйИдентификатор);
		УдалитьФайлы(Объект.ExchangeFileName);
...
Процедура УстановитьДоступностьКомандОтладки()
	...
	Элементы.НастройкаОтладкиЗагрузки.Доступность = Объект.HandlersDebugModeFlag;
	Элементы.НастройкаОтладкиВыгрузки.Доступность = Объект.HandlersDebugModeFlag;
...
//										  ПараметрыВыгрузки.ПодчиненныеЭлементы.ВыгрузкаВИБПриемник);
	...
	Объект.DirectReadFromDestinationIB = (ПрямаяВыгрузка = 1);
...
Процедура ТипИнформационнойБазыДляПодключенияПриИзмененииЗначения()
	...
	ТипБазы.ТекущаяСтраница = ?(Объект.ConnectedInfobaseType, ТипБазы.ПодчиненныеЭлементы.ФайловаяБаза,
...
Процедура ОткрытьФормуНастройкиОтладкиОбработчиков(ОбработчикиСобытийИзФайлаПравил)
	...
	ПараметрыФормы.Вставить("EventHandlerExternalDataProcessorFileName",
		Объект.EventHandlerExternalDataProcessorFileName);
	ПараметрыФормы.Вставить("AlgorithmDebugMode", Объект.AlgorithmDebugMode);
	ПараметрыФормы.Вставить("ExchangeRulesFileName", Объект.ExchangeRuleFileName);
	ПараметрыФормы.Вставить("ИмяФайлаОбмена", Объект.ExchangeFileName);
	...
	Если ПараметрыОтладки <> Неопределено Тогда
		...
		Если ЭтоКлиент Тогда
			...
			Если ОбработчикиСобытийИзФайлаПравил Тогда
				...
				ИмяФайла = Объект.ExchangeRulesFileName;

			Иначе

				ИмяФайла = Объект.ExchangeFileName;
...
Процедура ИзменитьРежимОбработки(РежимРаботы)
	...
	Объект.ExportRulesTable.ПолучитьЭлементы().Очистить();
	Объект.ParametersSettingsTable.Очистить();
...
Процедура ОткрытьДанныеПротоколовОбменаПриНеобходимости()
	...
	Если Не Объект.OpenExchangeLogAfterExecutingOperations Тогда
		Возврат;
	КонецЕсли;

#Если Не ВебКлиент Тогда

	Если Не ПустаяСтрока(Объект.ExchangeLogFileName) Тогда
		ОткрытьВПриложении(Объект.ExchangeLogFileName);
	КонецЕсли;

	Если Объект.DirectReadFromDestinationIB Тогда

		Объект.ImportExchangeLogFileName = ПолучитьИмяПротоколаДляВторойИнформационнойБазыComСоединенияНаСервере();

		Если Не ПустаяСтрока(Объект.ImportExchangeLogFileName) Тогда
			ОткрытьВПриложении(Объект.ImportExchangeLogFileName);
		КонецЕсли;
...
Процедура ПриИзмененииТипаУдаленияРегистрацииИзменений()
	...
	Если ПустаяСтрока(ТипУдаленияРегистрацииИзмененийДляУзловОбменаПослеВыгрузки) Тогда
		Объект.ChangesRegistrationDeletionTypeForExportedExchangeNodes = 0;
	Иначе
		Объект.ChangesRegistrationDeletionTypeForExportedExchangeNodes = Число(
...
Процедура ИзменитьРежимБезопаснаяЗагрузка(Интерактивно = Истина)
	...
	Элементы.ГруппаБезопаснаяЗагрузка.Доступность = Объект.SafeImport;
...
#КонецЕсли
	...
	Если Объект.SafeImport И ЧерезХранилище Тогда
		ПоместитьФайлПравилЗагрузкиВХранилище();
	КонецЕсли;
...
Функция ИнициализироватьКомпоновщикНастроекОтбораПравилВыгрузки(Знач ИндексСтроки = Неопределено,
	...
	Если СтрокаДереваМетаданных = Неопределено Тогда
		СтрокаДереваМетаданных = Объект.ExportRulesTable.НайтиПоИдентификатору(ИндексСтроки);
	КонецЕсли;
