...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Object.IsInteractiveMode = Истина;
	Object.SafeMode = Истина;
	Object.ExchangeLogFileEncoding = "КодировкаТекста.UTF8";
...
Процедура ВыполнитьЗагрузкуПравилОбмена(АдресФайлаПравилВХранилище = "", ИмяФайлаДляРасширения = "")
	...
	Иначе
		...
		УстановитьПризнакЗагрузкиПравил(Истина);
		РазвернутьСтрокиДерева(Object.ExportRulesTable, Элементы.ТаблицаПравилВыгрузки, "Включить");
...
Процедура УстановитьДоступностьКомандОтладки()
	...
	Элементы.НастройкаОтладкиЗагрузки.Доступность = Object.HandlersDebugModeFlag;
	Элементы.НастройкаОтладкиВыгрузки.Доступность = Object.HandlersDebugModeFlag;
...
Процедура ИзменитьРежимОбработки(РежимРаботы)
	...
	ГруппаРежима.ФормаНаКлиенте.Check = РежимРаботы;
	ГруппаРежима.ФормаНаСервере.Check = Не РежимРаботы;
